...
function create_password_file() {
  ...
  local pass_exist=$(op document list --vault "mna-vault-passwords-common" | grep ".vault-password-${PRODUCT_NAME}")
  if [[ "$pass_exist" != "" ]]; then
...
#   # ===> set dans bal (utile pour le deploy)
  ...
  local habilitations_exist=$(op document list --vault "mna-vault-passwords-common" | grep "habilitations-${PRODUCT_NAME}")
